...
#             print(dog_images_in_octave[-1])
            print('dog: ', dog_images_in_octave[-1].shape)
            plt.imshow(cv2.cvtColor(dog_images_in_octave[-1]*255, cv2.COLOR_BGR2RGB))
            plt.show()
        ...
        if region_y > 0 and region_y < image_shape[0] - 1:
            for j in range(-radius, radius + 1):
                ...
                if region_x > 0 and region_x < image_shape[1] - 1:
                    ...
                    gradient_magnitude = sqrt(dx * dx + dy * dy) #bien do
                    gradient_orientation = rad2deg(arctan2(dy, dx)) #tinh goc
